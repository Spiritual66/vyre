...
module.exports = (io) => {
  ...
  router.get('/', auth, (req, res) => {
    ...
    `).all(req.user.id, now, req.user.id, req.user.id, req.user.id);

    const grouped = {};
    for (const s of statuses) {
      if (!grouped[s.user_id]) grouped[s.user_id] = { user_id: s.user_id, username: s.username, avatar: s.avatar, statuses: [] };
